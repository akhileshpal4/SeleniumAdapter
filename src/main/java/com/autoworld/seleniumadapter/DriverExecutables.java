...
public class DriverExecutables {
    ...
    public static final String PROXY_FULL_URL;
    ...
    static {
        PROXY_URL = ConfigProvider.getAsString("proxy.url");
        PROXY_PORT = ConfigProvider.getAsString("proxy.port");
        PROXY_FULL_URL = PROXY_URL + ":" + PROXY_PORT;
    ...
    protected static void fnCallChromeWebDriverManager() {
        if (CHROME_VERSION != null && CHROME_VERSION.isEmpty()) {
            if (PROXY_VALUE) {
                WebDriverManager.chromedriver().clearDriverCache().browserVersion(CHROME_VERSION).proxy(PROXY_FULL_URL).proxyUser(PROXY_UNAME).proxyPass(decryptedKey(PROXY_PWD)).setup();
            } else {
                WebDriverManager.chromedriver().clearDriverCache().browserVersion(CHROME_VERSION).setup();
            }
        } else if (PROXY_VALUE) {
            WebDriverManager.chromedriver().clearDriverCache().proxy(PROXY_FULL_URL).proxyUser(PROXY_UNAME).proxyPass(decryptedKey(PROXY_PWD)).setup();
        } else if (PROXY_URL != null && !PROXY_URL.isEmpty()) {
            WebDriverManager.chromedriver().clearDriverCache().proxy(PROXY_FULL_URL).setup();
        } else {
    ...
    protected static void executeEdge32Bit() {
        if (EDGE_VERSION != null && !EDGE_VERSION.isEmpty()) {
            if (PROXY_VALUE) {
                WebDriverManager.edgedriver().clearDriverCache().browserVersion(EDGE_VERSION).arch32().proxy(PROXY_FULL_URL).proxyUser(PROXY_UNAME).proxyPass(decryptedKey(PROXY_PWD)).setup();
            } else {
                WebDriverManager.edgedriver().clearDriverCache().browserVersion(EDGE_VERSION).arch32().setup();
            }
        } else if (PROXY_VALUE) {
            WebDriverManager.edgedriver().clearDriverCache().arch32().proxy(PROXY_FULL_URL).proxyUser(PROXY_UNAME).proxyPass(decryptedKey(PROXY_PWD)).setup();
        } else if (PROXY_URL != null && !PROXY_URL.isEmpty()) {
            WebDriverManager.edgedriver().clearDriverCache().arch32().proxy(PROXY_FULL_URL).setup();
        } else {
    ...
    protected static void executeEdge64Bit() {
        if (EDGE_VERSION != null && !EDGE_VERSION.isEmpty()) {
            if (PROXY_VALUE) {
                WebDriverManager.edgedriver().clearDriverCache().browserVersion(EDGE_VERSION).proxy(PROXY_FULL_URL).proxyUser(PROXY_UNAME).proxyPass(decryptedKey(PROXY_PWD)).setup();
            } else {
                WebDriverManager.edgedriver().clearDriverCache().browserVersion(EDGE_VERSION).setup();
            }

        } else if (PROXY_VALUE) {
            WebDriverManager.edgedriver().clearDriverCache().proxy(PROXY_FULL_URL).proxyUser(PROXY_UNAME).proxyPass(decryptedKey(PROXY_PWD)).setup();
        } else if (PROXY_URL != null && !PROXY_URL.isEmpty()) {
            WebDriverManager.edgedriver().clearDriverCache().proxy(PROXY_FULL_URL).setup();
        } else {
    ...
    protected static void executeFirefox32Bit() {
        if (FIREFOX_VERSION != null && !FIREFOX_VERSION.isBlank() && !FIREFOX_VERSION.isEmpty()) {
            if (PROXY_VALUE) {
                WebDriverManager.firefoxdriver().clearDriverCache().browserVersion(FIREFOX_VERSION).arch32().proxy(PROXY_FULL_URL).proxyUser(PROXY_UNAME).proxyPass(decryptedKey(PROXY_PWD)).setup();
            } else {
                WebDriverManager.firefoxdriver().clearDriverCache().browserVersion(FIREFOX_VERSION).arch32().setup();
            }
        } else if (PROXY_VALUE) {
            WebDriverManager.firefoxdriver().clearDriverCache().arch32().proxy(PROXY_FULL_URL).proxyPass(PROXY_UNAME).proxyPass(PROXY_PWD).setup();
        } else if (PROXY_URL != null && !PROXY_URL.isEmpty()) {
            WebDriverManager.firefoxdriver().clearDriverCache().arch32().proxy(PROXY_FULL_URL).setup();
        } else {
    ...
    protected static void executeFirefox64Bit() {
        if (FIREFOX_VERSION != null && !FIREFOX_VERSION.isBlank() && !FIREFOX_VERSION.isEmpty()) {
            if (PROXY_VALUE) {
                WebDriverManager.firefoxdriver().clearDriverCache().browserVersion(FIREFOX_VERSION).proxy(PROXY_FULL_URL).proxyUser(PROXY_UNAME).proxyPass(decryptedKey(PROXY_PWD)).setup();
            } else {
                WebDriverManager.firefoxdriver().clearDriverCache().browserVersion(FIREFOX_VERSION).setup();
            }
        } else if (PROXY_VALUE) {
            WebDriverManager.firefoxdriver().clearDriverCache().proxy(PROXY_FULL_URL).proxyUser(PROXY_UNAME).proxyPass(decryptedKey(PROXY_PWD)).setup();
        } else if (PROXY_URL != null && !PROXY_URL.isEmpty()) {
            WebDriverManager.firefoxdriver().clearDriverCache().proxy(PROXY_FULL_URL).setup();
        } else {
